...
Arkenos uses three `.env` files, one per service.
...
| `TWILIO_ACCOUNT_SID` | For telephony | From [Twilio Console](https://console.twilio.com) — required for buying/releasing numbers and auto-provisioning SIP trunks |
| `TWILIO_AUTH_TOKEN` | For telephony | From [Twilio Console](https://console.twilio.com) |
| `TWILIO_SIP_DOMAIN` | No | SIP domain override (auto-derived from `LIVEKIT_URL` if not set, e.g. `xyz.sip.livekit.cloud`) |
| `LIVEKIT_SIP_TRUNK_ID` | No | Outbound SIP trunk ID fallback (auto-provisioned on first outbound call if not set) |
