...
description: Connect Twilio phone numbers to Nenyax via LiveKit SIP — mostly automated.
---

# Telephony Setup

Connect real phone numbers to your Nenyax voice agents using Twilio and LiveKit SIP. Nenyax **auto-provisions** most of the SIP infrastructure — you just need accounts and credentials.
...
```
Inbound:  Caller --> Twilio --> Elastic SIP Trunk --> LiveKit SIP --> Nenyax Agent
...
When you buy or assign a number through the dashboard/API, Nenyax **automatically**:

| Resource | Auto-Created |
|----------|-------------|
| LiveKit Inbound SIP Trunk | Yes — "Nenyax Inbound" |
| LiveKit Dispatch Rule | Yes — routes to `nenyax-agent` |
| LiveKit Outbound SIP Trunk | Yes — "Nenyax Outbound" (on first outbound call) |
| Twilio Elastic SIP Trunk | Yes — "Nenyax Inbound" with LiveKit origination |
...
Open the **Nenyax Dashboard** → **API Keys** (under Build in the sidebar) and configure:
...
1. Open the **Nenyax Dashboard** → **Agents** → select your agent
...
If you already own a Twilio number and want to use it with Nenyax:
...
<AccordionGroup>
  <Accordion title="Call rings but agent doesn't pick up">
    **Most common cause:** The agent service isn't running.
    ...
    2. Verify in LiveKit Cloud → Telephony → Dispatch Rules that a rule exists with `nenyax-agent`
  ...
  <Accordion title="Inbound call doesn't ring / goes to voicemail">
    ...
    1. Go to **Twilio Console → Elastic SIP Trunking** — verify "Nenyax Inbound" trunk exists
  ...
  <Accordion title="Outbound call initiates but phone doesn't ring">
    ...
    2. Check **LiveKit Cloud → Telephony → SIP Trunks** — verify "Nenyax Outbound" exists with the correct Twilio termination address
...
|----------|------|---------|
| LiveKit Inbound Trunk | "Nenyax Inbound" | Receives SIP calls from Twilio |
| LiveKit Dispatch Rule | "Nenyax Dispatch" | Routes calls to `nenyax-agent` worker |
| LiveKit Outbound Trunk | "Nenyax Outbound" | Sends SIP calls to Twilio |
| Twilio Elastic SIP Trunk | "Nenyax Inbound" | Routes calls between Twilio and LiveKit |
| Twilio Credential List | "Nenyax LiveKit" | Authenticates LiveKit → Twilio for outbound |
