...
- Clerk keys (frontend), Postgres password (backend). All other provider keys are configured via the dashboard.
...
The agent requires **no `.env` file**. On startup it fetches all API keys (LiveKit, Google, Resemble, STT providers) from the backend dashboard. Make sure the backend is running first and provider keys are configured at **Settings > API Keys**.
...
<AccordionGroup>
  ...
  <Accordion title="Agent doesn't connect to calls">
    - Ensure the backend is running and all provider keys are configured at **Settings > API Keys**
