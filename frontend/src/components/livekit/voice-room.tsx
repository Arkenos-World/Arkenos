            ...
            segments.forEach(segment => {
                if (segment.final && segment.text.trim()) {
                    ...
                    const isAgent = participant?.isAgent ||
                        ...
                        participant?.identity?.toLowerCase().includes("nenyax");
            ...
            <Card className="w-full max-w-md">
                <CardContent className="pt-6">
                    <div className="text-center mb-4">
                        ...
                        <h3 className="text-xl font-semibold">{agentName || "Nenyax Agent"}</h3>
...
export function VoiceRoom({ token, serverUrl, roomName, onDisconnect, agentName }: { token: string; serverUrl: string; roomName: string; onDisconnect: () => void; agentName?: string }) {
    return (
        <LiveKitRoom
            ...
            className="h-full"
        >
            <RoomContent onDisconnect={onDisconnect} roomName={roomName} agentName={agentName || "Nenyax Agent"} />
        </LiveKitRoom>
    );
}
